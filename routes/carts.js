...
const { toDateString, toDatetimeString } = require(__dirname + "/../modules/date_tools");
...
router.post("/", async (req, res) => {
    ...
    const [[{ num }]] = await db.query(sql3, [product_id, fake_user]);
    if (num > 0) {
    ...
    const [r1] = await db.query(sql, [product_id]);
    if (!r1.length) {
        output.error = "沒有這個商品";
        return res.json(output);
    }
...
router.delete("/", async (req, res) => {
    ...
    await db.query(sql, [fake_user, product_id]);
